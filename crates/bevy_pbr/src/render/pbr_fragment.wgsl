...
// TODO: Transforming UVs mean we need to apply derivative chain rule for meshlet mesh material pass
    ...
    pbr_input.material.reflectance,*= specular * 0.5,;
        ...
        if ((flags & pbr_types::STANDARD_MATERIAL_FLAGS_SPECULAR_TINT_TEXTURE_BIT
) != 0u) {
            let specular_tint =
#ifdef MESHLET_MESH_MATERIAL_PASS
                textureSampleGrad(
...
#ifdef VERTEX_UVS
        if ((flags & pbr_types::STANDARD_MATERIAL_FLAGS_EMISSIVE_TEXTURE_BIT
) != 0u) {
    ...
    mat2x2(anisotropy_direction.xy, anisotropy_direction.yx * vec2(-1.0, 1.0)) * anisotropy_direction_from_texture,;
